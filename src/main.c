...
K_MSGQ_DEFINE(mouse_jiggle_time, 1, 2, 1);
...
static void input_cb(struct input_event *evt, void *user_data)
{
	...
	case INPUT_KEY_0:
		...
		k_msgq_put(&mouse_jiggle_time, &tmp, K_NO_WAIT);
		LOG_ERR("Jiggle time: %d", tmp);
		break;
	default:
	...
	if (k_msgq_put(&mouse_jiggle_time, &tmp, K_NO_WAIT) != 0) {
...
int main(void)
{
	...
	report[MOUSE_X_REPORT_IDX] = 10U * factor;
	report[MOUSE_Y_REPORT_IDX] = 0U;
	report[MOUSE_WHEEL_REPORT_IDX] = 0U;
	uint8_t jiggle_time = 40U;
	while (true) {
		factor = -factor;
		if (k_msgq_get(&mouse_jiggle_time, &jiggle_time, K_MSEC(1000)) != 0) {
			// TODO: handle something here
		}
		LOG_INF("Jiggle mouse every %d s", jiggle_time);
		ret = hid_int_ep_write(hid_dev, report, MOUSE_REPORT_COUNT, NULL);
		k_sleep(K_MSEC(jiggle_time * 1000U));
	}
	return 0;
}
